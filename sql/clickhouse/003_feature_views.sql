...
SELECT
    ...
    if(
        rowNumberInFrame() OVER w > 1 AND (timestamp_ms - lagInFrame(timestamp_ms, 1) OVER w) > 0,
...
SELECT
    ...
    if(
        rowNumberInFrame() OVER w > 1 AND price_velocity IS NOT NULL AND lagInFrame(price_velocity, 1) OVER w IS NOT NULL AND time_delta_ms > 0,
